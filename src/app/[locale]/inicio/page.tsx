    ...
    <>
      ...
      <noscript>
        <div style={{ padding: '80px 24px', maxWidth: 800, margin: '0 auto', color: '#f5f5f2', fontFamily: 'system-ui' }}>
          ...
          <ul>
            <li><a href={`/${locale}/casos/apoderapp`}>Apoderapp — {isES ? 'Cobro de cuotas por WhatsApp' : 'Fee collection via WhatsApp'}</a></li>
            <li><a href={`/${locale}/casos/schwager`}>Schwager — Voice AI {isES ? 'en minería' : 'in mining'}</a></li>
            <li><a href={`/${locale}/casos/sime`}>SIME — {isES ? '4,000 rutas digitales/día' : '4,000 digital routes/day'}</a></li>
            <li><a href={`/${locale}/casos/soma`}>SOMA — BI {isES ? 'operacional' : 'dashboards'}</a></li>
            <li><a href={`/${locale}/casos/speakly`}>Speakly — AI {isES ? 'copiloto educativo' : 'education copilot'}</a></li>
          </ul>
