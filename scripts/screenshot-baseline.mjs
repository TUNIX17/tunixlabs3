...
const OUT = join(ROOT, 'reports', 'baseline');
...
const pages = [
  ...
  { url: '/es/servicios/business-intelligence', name: 'servicio-bi', wait: 2000 },
...
for (const vp of [1440, 768, 375]) {
  ...
  for (const { url, name, wait } of pages) {
    try {
      ...
      await page.screenshot({ path: join(OUT, `${name}-${vp}w.png`), fullPage: true });
      console.log(`OK ${name} @ ${vp}w`);
    } catch (e) {
      console.error(`FAIL ${name} @ ${vp}w: ${e.message}`);
    }
  }
...
await browser.close();
console.log(`\nBaseline screenshots saved to ${OUT}`);
